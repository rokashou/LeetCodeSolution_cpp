...
public:
    int maximumSafenessFactor(vector<vector<int>> &grid)
    {
        ...
        // Calculate safeness factory for each cell using BFS
        ...
        // Priority quere to prioritize cells with higher safeness factor
        ...
        grid[0][0] = -1; // Mark the source cells as visited

        // BFS to find the path with maximum factor
        while(!pq.empty()){
            auto curr = pq.top();
            pq.pop();

            // If reached the destiantion return safeness factor
