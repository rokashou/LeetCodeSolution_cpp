...
/* the key is "the origin array is sorted and included only prime numbers" */

// Find no of fractions less than mid, let this be total.
// Find i and j for maximum fraction less than mid.
// a) If total is equal to k, then the maximum fraction less than mid is our required answer. So we simply return this i and j found in step 2.
// b) If total is greater than k, no of fractions less than mid is greater so we set high to mid.
// c) else we set low to mid.

// as we have to find the kth smallest so if we keep record of max of all fractors when we do total == k we will get kth smallest only as we would have already stired it above by that time
