...
private:
    ...
    bool isVaildWord(vector<int>& subsetLetters){
        for (int c = 0; c < 26;c++){
    ...
    void check(int w, vector<string>& words, vector<int>& score, vector<int>& subsetLetters, int totalScore){
        ...
        check(w - 1, words, score, sub, totalScore);
...
public:
    int maxScoreWords(vector<string> &words, vector<char> &letters, vector<int> &score)
    {
        ...
        vector<vector<int>> wf(words.size(), vector<int>(26, 0));
        vector<int> ws(words.size(), 0);
