...
public:
    ...
    int shortestPathLength(vector<vector<int>> &graph)
    {
        ...
        queue<tuple> q;       // queue of clss tuple type
        ...
        // we don't know which node will give us shortest path,
        // so initially for all nodes we will store in our queue
        for (int i = 0; i < n;i++)
        {
            int maskValue = (1 << i); // 2^i
            tuple thisNode(i, maskValue, 1); // make a tuple for every node
            q.push(thisNode);
        ...
        while(q.empty() == false) // until queue is not empty
        {
            ...
            for(auto &adj: graph[curr.node])
            {
                ...
                // we are moving from one node on anthor node
